...
class Client(object):
    ...
    def RLU(self, global_weights):
        ...
        for batch_idx, (inputs, targets) in enumerate(self.trainloader):
            ...
            w_grad_epochs += w_grad
            count += 1
            # print("Probs: ", np.round((probs * 100).detach().cpu().numpy(), 2))
            # print("B Grad", b_grad)
    ...
    def overall_attacks(self, global_weights):
        ...
        for batch_idx, (inputs, targets) in enumerate(self.trainloader):
            ...
            if count == self.args.local_epochs:
                ...
                result_Acc['VLI'], result_irec['VLI'], result_cAcc['VLI'] = self.accuracy_calculation(VLI_attack( b_grad_epochs, self.args), existences, num_instances)
