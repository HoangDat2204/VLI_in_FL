...
class CUB200(Dataset):
    def __init__(self, root, train=True, transform=None):
        ...
        # Đường dẫn tới các file thông tin của CUB
        ...
        # Ghép thông tin lại với nhau
        for img, lbl, spl in zip(images, labels, splits):
            img_id, img_path = img
            _, label = lbl
            _, is_train = spl
            
            # is_train: '1' là tập train, '0' là tập test
            if (self.train and is_train == '1') or (not self.train and is_train == '0'):
                self.data.append({
                    'path': os.path.join(root, 'images', img_path),
                    # PyTorch yêu cầu label bắt đầu từ 0 (0-indexed), trong khi CUB bắt đầu từ 1
                    'label': int(label) - 1 
                })
